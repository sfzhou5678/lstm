...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Train"):
            train_input = InitInput(config=config, data=train_data, name="TrainInput")
            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                m = InitModel(is_training=True, config=config, input_=train_input,
                             START_MARK=START_MARK, END_MARK=END_MARK, PAD_MARK=PAD_MARK)
            tf.summary.scalar("Training Loss", m.cost)
            tf.summary.scalar("Learning Rate", m.lr)

        # with tf.name_scope("Valid"):
        #     valid_input = InitInput(config=config, data=valid_data, name="ValidInput")
        #     with tf.variable_scope("Model", reuse=True, initializer=initializer):
        #         mvalid = InitModel(is_training=False, config=config, input_=valid_input)
        #     tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("Test"):
            test_input = InitInput(config=eval_config, data=test_data, name="TestInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mtest = InitModel(is_training=False, config=eval_config, input_=test_input,
                                 START_MARK=START_MARK, END_MARK=END_MARK, PAD_MARK=PAD_MARK)
...
def decode():
    ...
    while True:
        with tf.Graph().as_default():
            ...
            with tf.name_scope("Train"):
                decode_input = InitInput(config=config, data=token, name="TrainInput", isDecode=True)
                with tf.variable_scope("Model", reuse=None, initializer=initializer):
                    decode_model = InitModel(is_training=True, config=config, input_=decode_input,
                                            START_MARK=START_MARK, END_MARK=END_MARK, PAD_MARK=PAD_MARK)
                # tf.summary.scalar("Training Loss", m.cost)
                # tf.summary.scalar("Learning Rate", m.lr)
        ...
        try:
            ...
            with tf.Graph().as_default():
                initializer = tf.random_uniform_initializer(-config.init_scale,
                                                            config.init_scale)
                with tf.name_scope("Train"):
                    decode_input = InitInput(config=config, data=testInput, name="TrainInput",isDecode=True)
                    with tf.variable_scope("Model", reuse=None, initializer=initializer):
                        decode_model = InitModel(is_training=True, config=config, input_=decode_input,
                                                START_MARK=START_MARK, END_MARK=END_MARK, PAD_MARK=PAD_MARK)
